...
def plot(fig_folder_name, all_action, node_data, demand_data, episode, problem_size, basename):
    ...
    for i in range(node_data.shape[0]):
        ...
        # 追加した部分
        if set(np.array(part_action.cpu().detach())) != {i for i in range(problem_size + 1)}:
        ...
        filename = f"{fig_folder_name}/{basename}_{str(i).zfill(4)}_{str(episode).zfill(4)}.png"
        print(filename)
...
def main():
    ...
    inference_data = "map_data_osmnx/paris,france_9484.parquet_200node_128num/test_location.pickle"
    ...
    while True:
        ...
        demand_data = Tensor(np.ones(TEST_BATCH_SIZE*PROBLEM_SIZE) / 50)
        ...
        plot(fig_folder_name, all_action, node_data, node_demand, episode, PROBLEM_SIZE, basename)
